    ...
    <template #default>
      <div class="bg-surface-base">
        ...
        >
          <span
            ...
            :class="[
              ...
              {
                'hover:bg-surface-gray-3 cursor-pointer': isClickableChip(
                  chip.key,
                ),
              },
            ]"
        ...
        >
          ...
          >
            {{
              isContactOp && contactSearch.loading
                ? __("Loading…")
                : __("No matches")
            }}
          </div>
        ...
        <div v-else-if="results?.data?.[0]?.length" class="px-2 pb-2">
          ...
          >
            <div class="mr-2 space-y-1 truncate">
              <p
                class="truncate text-base-semibold"
                v-html="highlight(result.subject || __('[No subject]'))"
              />
              <div class="flex items-center gap-1 truncate text-sm">
                <span
                  class="truncate"
                  v-html="highlight(getInterlocutors(result))"
                />
...
const showAdvancedFilters = defineModel<boolean>("showAdvanced", {
  default: false,
});
...
const isContactOp = computed(
  () => !!opTrigger.value && CONTACT_OPS.includes(opTrigger.value.op),
);
...
const contactMatches = computed(() => {
  ...
  const hasExact = matches.some(
    (c) => c.email.toLowerCase() === partial.toLowerCase(),
  );
...
const applyContact = (contact: { email: string }) => {
  if (opTrigger.value)
    (filter as Record<string, string>)[opTrigger.value.op] = contact.email;
  stripToken();
};
const stripToken = () => {
...
const QUICK_FILTERS = computed<QuickFilter[]>(() =>
  [
    ...
    { label: __("To"), key: "to", op: "to" },
    {
      ...
      key: "hasAttachment",
      apply: () => (filter.hasAttachment = "true"),
    },
    {
      label: __("Unread"),
      key: "isRead",
      apply: () => (filter.isRead = "false"),
    },
...
const EDITABLE_OP: Record<string, string> = {
  inMailbox: "in",
  from: "from",
  to: "to",
  cc: "cc",
...
const isClickableChip = (key: string) =>
  !!EDITABLE_OP[key] || isToggleableChip(key);
  ...
  Object.entries(filteredFilter.value)
    ...
    .map(([key, value]) => {
      ...
      if (key === "hasAttachment")
        return { key, label: optionLabel(getAttachmentOptions(), value) };
      if (key === "isRead")
        return { key, label: optionLabel(getReadStatusOptions(), value) };
      const display =
        key === "inMailbox" ? optionLabel(mailboxOptions.value, value) : value;
...
const noOfAttachments = (result) =>
  result.attachments?.filter(
    (m) => m.filename && m.disposition === "attachment",
  ).length || 0;
