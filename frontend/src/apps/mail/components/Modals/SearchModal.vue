		...
		<div class="bg-surface-base">
			...
			<div class="space-y-4 p-4">
				<Switch
					...
					:description="__('Look through every account you own — slower, but finds a mail wherever it landed.')"
					class="!p-0"
				/>
				<FormControl
					v-if="!allAccounts"
				...
				<div class="flex space-x-4">
					<FormControl v-model="filter.after" type="date" :label="__('From Date')" class="w-full" />
					<FormControl v-model="filter.before" type="date" :label="__('To Date')" class="w-full" />
				</div>
				<div class="flex space-x-4">
					<FormControl
			...
			<div v-if="!isMobile" class="flex w-full justify-end space-x-4 p-4">
				...
				<Button :label="__('Search')" variant="solid" class="w-28" @click="openSearchPage" />
			</div>
		</div>
	</component>
</template>
...
const initialFilters = defineModel<Record<string, string>>('initialFilters', { default: {} })
...
const getDefaultFilter = (reset = false) =>
	Object.fromEntries(
		filterKeys.map((key) => [
			key,
			reset ? '' : key === 'text' ? initialText.value || route.query[key] || '' : route.query[key] || '',
		]),
	)
...
const allAccounts = ref(
	hasMultipleAccounts.value &&
		(route.query.all_accounts != null || localStorage.getItem(ALL_ACCOUNTS_STORAGE_KEY) === 'true'),
)
