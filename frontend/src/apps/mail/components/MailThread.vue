<template>
  <div v-if="threadID" class="relative flex h-full flex-col overflow-hidden">
    <ThreadHeader
      ...
      @move-thread="
        (moveToMailbox: string) => emit('moveThread', moveToMailbox)
      "
      @add-thread-to-mailbox="
        (mailboxId: string) => emit('addThreadToMailbox', mailboxId)
      "
    ...
    <div class="relative min-h-0 flex-1 overflow-hidden">
      <Transition :name="slide || ''" @after-enter="emit('slideDone')">
        <div :key="isMobile ? threadID : 'thread'" class="flex h-full flex-col">
          ...
          <div
            v-if="isMobile && thread?.length"
            class="shrink-0 border-b px-3.5 pb-3.5 pt-1.5"
          >
                ...
                <template v-for="mail in group.mails" :key="mail.name">
                  ...
                  >
                    <ThreadDivider
                      :message="
                        __('{0} more messages', [
                          String(collapsedMailNames.size),
                        ])
                      "
                  ...
                  <div
                    v-if="
                      !collapsedMailNames.has(mail.name) && !isPoppedOut(mail)
                    "
                    ...
                    :class="{
                      ...
                      'sm:px-4 sm:py-5':
                        (thread.length > 1 || mail.draft) && !isCollapsed(mail),
                    ...
                    <template v-else-if="!mail.name.startsWith('draft')">
                      ...
                      >
                        ...
                        <MailActions
                          ...
                          @move-mail="
                            (m: Mail, target: string) =>
                              emit('moveMail', m, target)
                          "
                          @mark-mail-spam="
                            (m: Mail, spam: boolean) =>
                              emit('markMailSpam', m, spam)
                          "
                      ...
                      <div
                        ...
                        :class="{
                          ...
                          'pb-2':
                            isCollapsed(mail) && isMobile && !!mail.preview,
                        }"
                        @click.stop="mail.collapsed = !mail.collapsed"
                      >
                        <Avatar
                          ...
                          :size="
                            isCollapsed(mail) ? (isMobile ? 'lg' : 'md') : 'xl'
                          "
                        />
                        ...
                        <div
                          ...
                          :class="{
                            'items-baseline': isCollapsed(mail) && !isMobile,
                          }"
                        >
                          <div
                          ...
                          >
                            <div class="flex items-center space-x-1.5">
                              ...
                              >
                                <span>&lt;</span>
                                <Tooltip
                                  :text="__('Filter messages from this sender')"
                                >
                                  <span
                                    class="cursor-pointer hover:underline"
                                    @click.stop="
                                      filterBySender(mail.from_email)
                                    "
                      ...
                      <MailDetails
                        v-if="
                          !isCollapsed(mail) && showMailDetails === mail.name
                        "
                        :mail
                        class="mb-4"
                      />
...
const isSenderBlocked = (email: string) =>
  !!screenedAddresses.value.data?.some(
    (a: ScreenedAddress) =>
      a.action === "Reject" && matchesScreenedValue(email, a.email),
  );
...
const blockRemoteImagesEnabled = computed(
  () => scope.account.value?.block_remote_images ?? true,
);
...
const effectiveScreenedAddresses = computed(() => {
  ...
  for (const a of globalScreenedAddresses.data ?? [])
    merged.set(a.email.toLowerCase(), a);
  for (const a of screenedAddresses.value.data ?? [])
    merged.set(a.email.toLowerCase(), a);
...
const isScreenedIn = (email: string) =>
  ...
  effectiveScreenedAddresses.value.some(
    (a: ScreenedAddress) =>
      a.action === "Accepted" && matchesScreenedValue(email, a.email),
  );
...
const recipientsSummary = (mail: Mail) => {
  ...
  const names = ordered.map(
    (r: Recipient) => (r.display_name || r.email).split(" ")[0],
  );
...
const unseenCount = computed(
  () => (thread.value || []).filter((m) => !m.seen && !m.draft).length,
);
const firstUnseenMail = computed(
  () => thread.value?.find((m) => !m.seen && !m.draft)?.id,
);
...
const transformThreadMails = (mails: Mail[]) =>
  mails
    ...
    .filter(
      (mail) =>
        readonly || (!removedMailIds.has(mail.id) && filterRelevantMails(mail)),
    )
    .map((mail) => ({
      ...mail,
...
const loadThread = () => {
  ...
  if (
    held &&
    data.some((mail: Mail) => mail.id === held) &&
    !data.some(isPoppedOut)
  )
    closeComposeWindow();
...
const setCollapsedGroup = (data: Mail[]) => {
  ...
  const triggerIdx = data.findIndex(
    (m) => m.name === firstMailOfCollapsedGroup.value,
  );
  mailBeforeCollapsedGroup.value =
    triggerIdx > 0 ? data[triggerIdx - 1].name : null;
};
...
const filteredAttachments = (mail: Mail) =>
  mail.attachments.filter(
    (a: Attachment) =>
      a.disposition === "attachment" || !a.type.startsWith("image/"),
  );
...
const showsDsnCard = (mail: Mail) =>
  !readonly && !isCollapsed(mail) && !!mail.dsn_blob_id;
...
const downloadAttachmentsAsZip = async (mail: Mail) => {
  ...
  try {
    const url = await getAttachmentsZipUrl(
      mailAttachments,
...
const lastMessage = computed(
  () =>
    [...thread.value].reverse().find((mail: Mail) => !mail.draft) ??
    thread.value.at(-1),
);

const isCollapsed = (mail: Mail) =>
  !!(mail.collapsed && mail !== lastMessage.value);
...
const syncMailboxMembership = (mailboxId: string, add: boolean) => {
  if (add) {
    const mb = scope.mailboxes.value.data?.find(
      (m: MailboxData) => m.id === mailboxId,
    );
    if (!mb) return;
    const entry: Mailbox = {
      mailbox: mb.name,
    ...
    thread.value?.forEach((mail: Mail) => {
      if (!mail.mailboxes.some((m) => m.mailbox_id === mailboxId))
        mail.mailboxes.push(entry);
    });
  } else if (thread.value?.every((mail: Mail) => mail.mailboxes.length > 1))
    thread.value?.forEach(
      (mail: Mail) =>
        (mail.mailboxes = mail.mailboxes.filter(
          (m) => m.mailbox_id !== mailboxId,
        )),
    );
};
...
const getReplyDetails = (mail: Mail) => ({
  ...
  attachments:
    mail.attachments?.filter((a: Attachment) => a.disposition === "inline") ||
    [],
  in_reply_to: mail.message_id,
  in_reply_to_id: mail.id,
});
...
const getIdentityEmail = (email?: string) =>
  identities.value.data?.find(
    (i: Identity) => i.email.toLowerCase() === email?.toLowerCase(),
  )?.email;
...
const getReplyIdentityEmail = (mail: Mail) => {
  ...
  return getIdentityEmail(
    [mail.from_email, ...[...to, ...cc, ...bcc].map((r) => r.email)].find(
      isUserEmail,
    ),
  );
};
...
const getPlainTextBody = (mail: Mail) =>
  decodeHtmlEntities(mail.html_body || mail.text_body || "");
