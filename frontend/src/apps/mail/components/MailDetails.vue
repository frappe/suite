<template>
  <div
    class="overflow-y-auto rounded-4 border text-sm sm:max-h-96 sm:w-96 sm:border-0"
  >
    <div class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-2 px-4 py-3">
      <template v-for="field in fields" :key="field.label">
        ...
        <span class="text-ink-gray-7 text-p-sm break-words">{{
          field.value
        }}</span>
      </template>
    </div>
...
const recipients = computed(() =>
  getGroupedRecipients(mail.recipients, true, true),
);
