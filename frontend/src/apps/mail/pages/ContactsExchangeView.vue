<template>
	<div v-if="contactsExchange.data" class="flex h-screen flex-col">
		...
		<div class="mx-auto my-5 rounded-4 border p-12 sm:w-[60rem]">
			<div class="flex items-center space-x-2">
				...
				<Badge
					:theme="getTheme(contactsExchange.data?.status)"
					:label="contactsExchange.data?.status"
				/>
			</div>
			<p class="my-4 text-base">{{ operationDetails }}</p>
			<CopyCode
				v-if="contactsExchange.data?.output"
				:code="contactsExchange.data?.output"
...
const dropdownOptions = computed(() => [
	{
		...
		onClick: () => window.open(`/app/contacts-exchange/${id}`, '_blank')?.focus(),
	},
])

const breadcrumbs = computed(() => [
	{
		label: __('Contacts Exchanges'),
		route: `/mail/contacts-exchanges?operation=${contactsExchange.data?.operation}`,
	},
	{ label: id },
])
</script>
