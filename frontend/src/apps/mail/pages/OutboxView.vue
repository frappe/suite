<template>
  <div class="flex h-full flex-col">
    ...
    <div
      class="flex-1 overflow-y-auto px-3 py-2.5 sm:px-5"
    ...
    >
      <template v-if="!refetching">
        ...
        >
          ...
          <div class="text-ink-gray-5 mt-1 text-base">
...
const hasActiveFilters = computed(
  () => activeSubmissionFilterCount(filters) > 0,
);
...
watchDebounced(() => [filters.emailId, filters.threadId], restart, {
  debounce: 300,
});
watch(
...
> = {
  pending: {
    ...
    description: __(
      "Scheduled emails and deliveries still in flight will wait here.",
    ),
  },
  final: {
    title: __("No final submissions"),
    description: __(
      "Concluded deliveries — delivered, sent, or failed — will appear here.",
    ),
  },
  canceled: {
...
const rescheduleMail = createResource({
  ...
  onSuccess: (data: { send_at: string }) => {
    refresh();
    raiseToast(
      __("Delivery rescheduled to {0}.", [formatDateTime(data.send_at)]),
    );
  },
...
const sendNowOptions = computed(() => ({
  title: __("Send Now"),
  message: __(
    "Deliver this email immediately instead of at the scheduled time?",
  ),
  actions: [
    {
      label: __("Send"),
