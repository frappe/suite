          ...
          <ListRowItem :item="item">
            ...
            <template v-else-if="column.key === 'storage'">
              ...
              <Tooltip v-else :text="storageTooltip(row.quota)">
                <span v-if="row.quota.unlimited" class="text-ink-gray-5 text-sm"
                  >∞</span
                >
                <div v-else class="flex items-center gap-2">
                  <div
                    class="bg-surface-gray-4 h-1.5 w-16 shrink-0 rounded-full"
                  >
                    <div
            ...
            <template v-else-if="column.key === 'last_active'">
              <span class="text-ink-gray-5 text-sm">
                {{ row.last_active ? fromNow(row.last_active) : __("Never") }}
              </span>
            </template>
          </ListRowItem>
        </ListRow>
...
const listOptions = computed(() => ({
  ...
  emptyState: hasActiveFilters.value
    ...
    : {
        ...
        description: __(
          "Invite people to give them a mailbox on your domains.",
        ),
      },
...
const ENABLE_MEMBERS_OPTIONS = {
  ...
  actions: [
    { label: __("Confirm"), variant: "solid", onClick: enableMembers.submit },
  ],
};
...
const disableMembers = createResource({
  ...
  onError: (error: { messages?: string[] }) => {
    showDisableMembers.value = false;
    raiseToast(
      error.messages?.[0] || __("Failed to disable members."),
      "error",
    );
  },
});
...
const DISABLE_MEMBERS_OPTIONS = {
  ...
  actions: [
    { label: __("Confirm"), variant: "solid", onClick: disableMembers.submit },
  ],
};
