...
describe('the manifest', () => {
	it('names every reference once, in order, with no content', () => {
		...
		for (const row of manifest.references) {
			expect(Object.keys(row).sort()).toEqual(['index', 'presentation', 'reference'])
		}
	})
...
describe('a group', () => {
	it('answers one entry per requested id, in the order it was asked', () => {
		for (const group of groups) {
			expect(group.references.map((row) => row.reference)).toEqual(group.request.references)
		}
	})

	it('never asks for more references than the bound, and leaves a code for the composite', () => {
		for (const group of groups) {
			expect(group.request.references.length).toBeLessThanOrEqual(fixture.group_limit)
			expect(group.request.x_drive_links.length).toBeLessThanOrEqual(fixture.link_header_limit)
	...
	it('keeps a reference identifier and its index from the manifest', () => {
		const byId = new Map(manifest.references.map((row) => [row.reference, row]))
		for (const group of groups) {
	...
	it('gives a readable reference a node and slides', () => {
		const readable = groups.flatMap((group) => group.references).filter((row) => row.readable)
	...
	it('gives an unreadable reference a placeholder and no content at all', () => {
		const marked = groups.flatMap((group) => group.references).filter((row) => !row.readable)
...
describe('refusals', () => {
	...
	it('separates a request the client got wrong from an answer about access', () => {
		const shape = ['injected_reference', 'malformed_group', 'oversized_group', 'repeated_reference']
		for (const name of shape) {
			expect(refusals[name].exception).toBe('frappe.ValidationError')
		}
		expect(refusals.unreadable_composite.exception).toBe('frappe.PermissionError')
	})

	it('refuses an oversized group at the bound, not one past the header limit', () => {
		expect(refusals.oversized_group.request.references.length).toBe(fixture.group_limit + 1)
