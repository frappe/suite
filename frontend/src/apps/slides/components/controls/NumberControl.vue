...
const current = computed(() =>
  live.value !== null ? live.value : model.value,
);
...
function onArrowStep(event) {
  ...
  const typed =
    live.value !== null ? parseFloat(live.value) : Number(model.value);
  live.value = null;
...
const fieldClasses = computed(() => [
  "inline-flex items-center gap-0.5 rounded-1 p-1",
...
const inputWidth = computed(() => {
  ...
  const capped = props.maxDigits
    ? Math.min(wholeDigits, props.maxDigits)
    : wholeDigits;
