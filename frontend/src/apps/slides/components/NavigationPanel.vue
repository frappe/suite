  ...
  >
    ...
    >
      ...
      <span :class="labelClasses"
        >{{ (slideIndex ?? 0) + 1 }} of {{ slidesLength }}</span
      >
    </div>
    <div
      ref="scrollableArea"
...
const menuWrapper = computed(() =>
  inReadonlyMode.value ? "div" : ContextMenu,
);
...
const isSlideActive = (slide) =>
  slideIndex.value === slides.value.indexOf(slide);
...
const scrollToSlide = (index) => {
  ...
  const virtualItem = rowVirtualizer.value
    .getVirtualItems()
...
onActivated(() => {
  nextTick(() => {
    ...
    rowVirtualizer.value.scrollToIndex(slideIndex.value, {
      align: "start",
      behavior: "auto",
    });
  });
