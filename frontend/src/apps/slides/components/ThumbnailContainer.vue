...
const slideNumber = computed(() =>
  presentationDoc.value?.is_composite
    ? slides.value.indexOf(props.slide) + 1
    : props.slide.idx,
);

const isFocused = computed(
  () => focusedSlide.value == slides.value.indexOf(props.slide),
);
const usesSelectionRing = computed(
  () => (props.isActive && recentlyRestored.value) || isFocused.value,
);
...
const getThumbnailStyles = (s) => {
  return {
    ...
    ...(usesSelectionRing.value
      ? {
          "--tw-ring-color": `color-mix(in srgb, ${selectionColor} 60%, transparent)`,
        }
      : {}),
  };
};
