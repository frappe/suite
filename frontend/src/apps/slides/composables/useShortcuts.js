...
import {
  ...
  commitCrop,
  cancelCrop,
} from "@/apps/slides/stores/imageCrop";
...
export const useShortcuts = (inReadonlyMode, inSlideShowMode) => {
  const inEditMode = () =>
    !inReadonlyMode.value && !inSlideShowMode.value && !inCropMode.value;
  ...
  onUnmounted(() =>
    window.removeEventListener("keydown", handleTypeToEdit, true),
  );
  ...
  const shortcuts = [
    ...
    },
    {
      ...
      condition: inEditMode,
      handler: () => {
        if (hasActiveTextEditor() && !isSelectionLocked.value)
          toggleMark("italic");
      },
    },
    {
  ...
  useKeyboardShortcut(
    shortcuts.map(({ key, ctrl, shift, condition, ...shortcut }) => ({
      ...shortcut,
      combo: [ctrl && "Mod", shift && "Shift", key].filter(Boolean).join("+"),
      enabled: condition,
    })),
  );
};
