...
`158a173a1c8fb0083f2b352250f8ac4bba1781ba`. Status stays `in-progress`: the
integration tests need the shared bench site and have not run on this
revision.
...
- `suite/drive/_core/versions.py` adds `take_version`, `list_versions`,
  ...
  delete.
- A version row is immutable. The doctype controller refuses any change to
  `node`, `seq`, `kind`, `actor`, `size`, `blob`, and `creation` after insert,
  so only `label` and `pinned` stay mutable.
...
- **The content callback's MIME.** `version_bytes` returns `(stream, mime)`
  ...
  `Drive Node Version` has no MIME column in §3.4, and
  `frappe.storage.blob.put_blob` sniffs the bytes with no caller override, so
  a JSON or plain text body is stored as `application/octet-stream`. Nothing
  is lost: the §11.2 version download route signs a URL with a filename, and
  `frappe/storage/serve.py` recovers the type from that filename when the
  stored blob sniffed as octet-stream. A MIME column and a `put_blob`
  parameter would each amend an accepted decision, so neither was added.
...
```text
uvx ruff@0.12.3 check suite/drive/_core/versions.py suite/drive/_core/roots.py suite/drive/_core/nodes.py suite/drive/jobs.py suite/drive/tests/test_versions.py suite/drive/doctype/drive_node_version/drive_node_version.py
uvx ruff@0.12.3 format --check <the same files>
python -m compileall -q <the same files> suite/hooks.py
git diff --check
```
