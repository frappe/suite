...
- Pass 1, the storage_v2 setup, and the index, Removed-content, legacy-comment, slides-media, borrowed-media, slide-elements and template-node fixes are done above. After the thread-names fix merges: flush redis, same migrate command, expect Build to finish the strict history call, the deferred deck, and steps 11-13; then read the report and reconcile against `source-inventory.md`.
