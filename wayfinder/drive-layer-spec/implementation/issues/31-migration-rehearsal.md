...
- Pass 1, the storage_v2 setup, and the index, Removed-content, legacy-comment, slides-media, borrowed-media and slide-elements fixes are done above. After the template-node fix merges: apply `pass9-precorrection.sql`, flush redis, same migrate command, expect Build to resume at step 8 and finish; then read the report and reconcile against `source-inventory.md`.
