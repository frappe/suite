...
This repository-local snapshot supplies the storage assumptions used by
[`../drive-layer-spec.md`](../drive-layer-spec.md) §13. The Drive spec owns the
exact integration signatures and tests. This document does not define Suite
module boundaries; those come from
[`../../../ARCHITECTURE.md`](../../../ARCHITECTURE.md).

## Drive integration amendment — 2026-09-05

The Drive rewrite keeps the storage model below and requires seven framework
changes. This is the synchronized index; the exact contracts are in the local
Drive spec §13.

| Ask | Framework change |
|---|---|
| 1. GC discovery | Public `blob_reference_columns()` and `orphan_predicate()` discover every Link to `File Blob`; an unindexed reference warns, and discovery failure deletes nothing. |
| 2. Finish to blob | Public `finish_upload_to_blob()` completes a resumable upload without creating a framework `File`. |
| 3. Signed blob URL | Public `signed_url_for_blob()` mints node-authorized egress without requiring a `File` row. |
| 4. Upload hook | `create_file_from_blob()` fires the existing `after_file_upload` hook. |
| 5. Range and streaming | Storage drivers expose ranged reads and serving exposes public `stream_blob()` while preserving the `serve_file` permission check. |
| 6. Relocation | Public, resumable `relocate_blobs()` moves stored blobs to the configured driver without changing references. |
| 7. Drive upload gates | Upload creation and finish accept explicit `check_permission` and `restrict_mimetypes` controls so Drive can apply its own access and MIME policy, including Share Link uploads. |
