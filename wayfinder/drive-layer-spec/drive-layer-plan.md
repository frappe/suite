...
- Suite repo: `/home/faris/benches/suite-bench/apps/suite`
...
- Framework repo: `/home/faris/benches/suite-bench/apps/frappe`
- Framework branch: `forge/storage-v2` (already checked out). The §13 asks
  land there first.
- Bench: `/home/faris/benches/suite-bench`, webserver port 8010
...
| Fact | Command |
|---|---|
| both branches | `bench version` in `/home/faris/benches/suite-bench` |
...
`fw` framework, `eng` engine, `node` node SDK, `side` side tables,
`http`, `dav`, `writer`, `slides`, `sheets`, `patch`.
...
| Path | Owner | Content |
|---|---|---|
| `sdk/__init__.py` | eng | **new.** Re-exports the public SDK |
| `sdk/roles.py` | eng | **new.** Role constants, `PTYPE_ROLE` (§4.1, §4.3) |
| `sdk/principals.py` | eng | **new.** `Principals`, `principals_for_request`, `X-Drive-Links`, unlock tickets (§4.6, §4.7, §4.8) |
| `sdk/access.py` | eng | **new.** `chain_ids`, `Acc`, `effective_role`, `effective_roles`, `check`, `require`, `explain`, `grant`, `revoke`, `revoke_or_deny`, `revoke_below`, `rotate_link`, `unlock_link`, list predicates (§5) |
| `sdk/errors.py` | eng | **new.** Six exception classes with status codes (§11.6) |
| `sdk/nodes.py` | node | **new.** create per kind, `get`, `update`, `purge`, `copy`, `children`, `views` (§8) |
| `sdk/upload.py` | node | **new.** `create_upload`, `finish_upload` (§8.4) |
| `sdk/quota.py` | node | **new.** `admit`, `release`, `effective_quota`, `recompute`, reservations (§7) |
| `sdk/versions.py` | side | **new.** `take_version`, `restore_version`, `label_version`, `delete_version`, `thin` (§9.1) |
| `sdk/previews.py` | side | **new.** `enqueue_render`, `render`, `push_preview`, `sweep_missing` (§9.2) |
| `sdk/comments.py` | side | **new.** threads and comments (§9.3) |
| `sdk/activity.py` | side | **new.** `record`, recents, favourites, notifications (§9.4, §9.5) |
| `sdk/content.py` | writer | **new.** `ContentTypeSpec`, `Satellite`, `DriveContent`, registry, hook targets, `touch`, `list_media`, `sweep_unused_media` (§10) |
...
| `get.py`, `copy.py`, `structure.py`, `lock.py` | dav | Rewritten: every direct `manager.*` call goes; `copy.py` calls `nodes.copy` |
...
| Path | Owner | Content |
|---|---|---|
| `drive/patches/rename_entity_log_to_recent.py` | patch | **new.** Pre-model-sync rename (§14.1) |
| `drive/patches/build.py` | patch | **new.** The 13 Build steps (§14.2 to §14.9) |
| `drive/patches/cleanup.py` | patch | **new.** Gate and steps (§14.10) |
| `suite/patches.txt` | patch | Adds the rename under `[pre_model_sync]`, `build` and later `cleanup` at the end of `[post_model_sync]` |
| `suite/hooks.py` | http | `before_request` gains the translator beside the DAV dispatcher (line 336); `streaming_request_paths` gains `/api/suite/drive/uploads/` (line 341); `drive_content_types` is new; `has_permission` and `permission_query_conditions` retarget the content and satellite doctypes (lines 122 to 189); the four `sync_content_file` `doc_events` go (lines 233 to 243); `scheduler_events` gains six daily jobs (line 282); `ALLOWED_WILDCARD_PATHS` gains `/api/suite/drive/` and loses `/api/method/suite.drive.api.` in stage 7 (line 429) |
...
| Path | Owner | Change |
|---|---|---|
| `suite/writer/drive.py` | writer | **new.** `SPEC` plus `create_empty`, `duplicate`, `export`, `version_bytes`, `restore_version`, `on_purge`, `used_nodes` |
| `writer/doctype/writer_document/writer_document.json` | writer | Adds `node`; drops `ycomments` and `versions` (with its `Writer Doc Version` child table) |
| `writer/doctype/writer_document/writer_document.py` | writer | Adds the `DriveContent` mixin; deletes `new_version` (:40), `update_file` (:100), `save_comments` (:110) |
| `writer/overrides/__init__.py` | writer | Deletes `filter_templates`, `template_has_permission`, `version_has_permission`, `document_query_conditions`, `version_query_conditions` |
| `suite/slides/drive.py` | slides | **new.** `SPEC` with `pushes_preview=True` and a `Slide` satellite |
| `slides/doctype/presentation/presentation.json` | slides | Adds `node`; drops `title`, `is_template`, `thumbnail` |
| `slides/doctype/presentation/presentation.py` | slides | Deletes the composite public invariant (:29-40), the forced-public row (:47-61), the `is_template` early return (:43), thumbnail File handling (:109-189, :361-395), `get_permission_query_conditions` and `has_permission` (:490-498), the webp convert-and-delete (:581-625); rewrites `get_attachment`, `attach_poster`, `update_slide_attachments`, `get_updated_json` (:240-487) onto `nodes.copy` |
| `slides/api/file.py`, `slides/api/test_file.py` | slides | **deleted** (122 and 274 lines) |
| `suite/sheets/drive.py` | sheets | **new.** `SPEC` with `import_from_file` and two satellites |
| `sheets/doctype/sheet/sheet.json` | sheets | Adds `node`; drops `title`, `trashed`, `trashed_on`, `trashed_by`, `head_snapshot` |
| `sheets/doctype/sheet/sheet.py` | sheets | Adds the mixin; deletes `after_insert`'s `create_drive_file` (:55) |
| `sheets/permissions.py`, `sheets/trash.py` | sheets | **deleted** (both replaced by the engine and the trash sweep) |
| `sheets/api.py` | sheets | Deletes `share_sheet`, `unshare_sheet`, `get_sheet_shares` |
| `sheets/collab.py` | sheets | `check_collab_access` resolves through the SDK; re-check every 5 minutes (§6.7) |

Frontend files named by ticket 012 (`canonicalMediaKey.ts`, `mediaUploads.js`,
`slidesRequests.js`) are out of scope: §1 makes frontend work a later effort.
...
| Path | Owner | Note |
|---|---|---|
| `drive/overrides/file.py` | http | Deleted whole with the `File` override, except `get_file_for_doc`, which is permanent (§11.7) |
| `drive/api/files.py`, `list.py`, `permissions.py`, `activity.py`, `notifications.py`, `storage.py`, `scripts.py`, `embed.py` | http | Bodies become forwarders in stage 4, deleted in stage 7 |
| `drive/api/s3.py` | http | Kept. `fetch` is permanent |
| `drive/api/product.py` | http | Kept. 19 methods, none touches a node |
| `drive/tests/test_sync_permissions.py` | http | **deleted** in stage 4. It covers `sync_from_disk`, which Build replaces, and `sync_preview`, which becomes a route (§11.7) |
| `drive/api/storage.py` | node | `acquire_owner_storage_lock` (:12) and `validate_quota` go in stage 2; the admission UPDATE is the lock (§7.2) |

Meet is the only caller outside Drive. Those are not Drive files, so they
change in stage 2 beside the SDK move (§7.8):

| Path | Owner | Change |
|---|---|---|
| `suite/meet/api/recording.py` | node | Drops `acquire_owner_storage_lock` (:17, :413, :776); `create_storage_reservation` (:453), `grow_storage_reservation` (:780), and `get_storage_usage` (:73) import from `sdk/quota.py` and name a root (§3.12) |
| `suite/meet/recording/ingest.py` | node | Same: lock at :19, :200; `reduce_storage_reservation` at :94 |
| `suite/meet/doctype/meet_recording/meet_recording.py` | node | `release_storage_reservation` (:277) imports from `sdk/quota.py` |
...
- **Entry:** stage 0 asks 1 to 3 committed. Branch `forge/drive-layer` created
  off `forge/wayfinder-drive-layer`.
- **Files:** the nine new doctype directories, the eight reshaped ones,
  `sdk/roles.py`, `sdk/principals.py`, `sdk/access.py`, `sdk/errors.py`,
  `sdk/__init__.py`, `patches/rename_entity_log_to_recent.py`, its
  `patches.txt` entry.
- **Roles:** doctype permission rows. Content doctypes keep a wide-open `All`
  row (§10.4); Drive doctypes are reached through the SDK only.
...
### Stage 2. Node SDK, upload, quota, versions, previews, activity

- **Entry:** stage 1 green. Stage 0 asks 2, 3, and 5 available on the bench.
- **Files:** `sdk/nodes.py`, `sdk/upload.py`, `sdk/quota.py`,
  `sdk/versions.py`, `sdk/previews.py`, `sdk/comments.py`, `sdk/activity.py`,
  the six daily `scheduler_events` in `hooks.py`,
  `streaming_request_paths`, the four Meet files above. Deletes
  `acquire_owner_storage_lock` and `validate_quota` from
  `drive/api/storage.py` and its six call sites in `api/files.py`,
...
- **Entry:** stage 2 green.
- **Files:** `sdk/content.py`, `suite/writer/drive.py`,
  ...
  deletion. Deletes `slides/api/file.py`, `slides/api/test_file.py`,
  `sheets/permissions.py`, `sheets/trash.py`, and the listed methods in
  `writer/overrides/__init__.py`, `presentation.py`, `sheet.py`,
  `sheets/api.py`.
...
- **Files:** `http/translator.py`, `http/routes.py`, `http/shapes.py`,
  ...
  in `hooks.py`. Deletes `drive/tests/test_sync_permissions.py`.
...
- **Entry:** stages 1 to 5 green. Stage 0 ask 1 committed, because Cleanup
  later gates on it.
- **Files:** `drive/patches/build.py`, its `patches.txt` entry,
  `test_build_patch.py`.
...
- **Entry:** all three §14.10 gates hold: every reachable Drive `File` row has
  ...
  old names.
- **Files:** `drive/patches/cleanup.py`, its `patches.txt` entry,
  `test_cleanup_patch.py`, the six deleted doctype directories, the deleted
  `drive/api/*` and `drive/overrides/file.py` bodies, `http/shims.py`, the
...
| `revoke_or_deny` is a new SDK name | §5.10 | The frozen `access.py` list has `revoke` and `revoke_below`. `revoke_or_deny` implements [007 §6]'s rule for every principal, not only `$PUBLIC`. Sign off before stage 1. |

## Rules for all agents

- Work only inside `/home/faris/benches/suite-bench/apps/suite` and
  `/home/faris/benches/suite-bench/apps/frappe`.
...
- Never edit `drive-layer-spec.md`, the tickets, `MAP.md`, or the explainers.
