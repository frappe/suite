...
def record_rsvp(account: str, event_id: str, response: str) -> str:
    ...
    The answer is the series'. An occurrence answered on its own is written as an override, and
    the server does not read that back the same way in every shape an invitation arrives in, so
    it is not offered — see the scope dialog.
    """
