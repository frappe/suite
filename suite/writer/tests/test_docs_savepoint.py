"""`writer.api.docs.create_document` reports its deadlock, not a lost savepoint.

`writer.api.docs.create_document` opens its own savepoint, calls `drive.create_document` inside it, and
...
class TestTheWriterCreateReportsItsDeadlock(UnitTestCase):
    def setUp(self):
        missing = object()
    ...
    def test_a_deadlocked_create_reports_the_deadlock_not_the_lost_savepoint(self):
        ...
        with (
            patch.object(docs.drive, "personal_root_for", return_value="root"),
            patch.object(docs.drive, "create_document", side_effect=self.deadlocked),
            self.assertRaises(frappe.QueryDeadlockError) as raised,
        ):
            docs.create_document(title="Untitled")
    ...
    def test_an_ordinary_failure_still_rolls_back_narrowly(self):
        ...
        with (
            patch.object(docs.drive, "personal_root_for", return_value="root"),
            patch.object(docs.drive, "create_document", side_effect=Refused("no upload rights")),
