...
**Storage Reservation** (shortened to **Reservation** below):
Bytes a Drive Root has promised to something not yet stored, counted as Usage
until the bytes land or the promise is let go.
_Avoid_: Budget, Hold, Pending upload, Lock, Temporary File

Meet's [Recording Budget](../meet/CONTEXT.md#language) is a separate concept:
the allowance for a Recording Session, persisted as `budget_bytes`. Meet backs
that budget with a Drive **Storage Reservation**, which accounts for the held
storage against the Drive Root's quota. Use each term in its own domain.
...
- A **Storage Reservation** can grow only when the owner has sufficient
  unreserved quota.
