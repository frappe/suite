...
class TestMigratedHistory(BuildContentCase):
    ...
    def test_both_adapters_restore_a_migrated_version(self):
        ...
        writer_drive.restore_version(self.prefix + "wdoc", io.BytesIO(target.read_blob(writer_blob)))
        sheets_drive.restore_version(self.prefix + "sheet", io.BytesIO(target.read_blob(sheet_blob)))
...
class TestContentLinksAndSources(BuildContentCase):
    ...
    def full_fixture(self) -> dict:
        ...
        skip for a `Presentation` template (`content.py:165-172`) and none
        for a Writer one. That is a step 10 defect, not a fixture choice:
        every Build on a site holding a Writer template stops there. Until
        it is fixed, a Writer template cannot appear in a `convert_all`
        fixture. `TestTemplates` covers both template types through step 8.
        """
        thread = self.prefix + "th"
