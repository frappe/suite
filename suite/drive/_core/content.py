...
MEDIA_ROW_FIELDS = ("name", "title", "kind", "blob", "size", "mime", "creation")
...
def _validate_satellite(satellite: Satellite, content_doctype: str) -> None:
    if satellite.link_field == "parent":
        # A child table names its parent through the framework's own column,
        # which carries no fieldname of its own on the child doctype.
...
class DriveContent:
    ...
    def __init_subclass__(cls, **kwargs) -> None:
        ...
        declared = cls.__dict__.get("before_insert")
        if declared is None or getattr(declared, "_drive_node_guard", False):
            return

        @functools.wraps(declared)
        def before_insert(self) -> None:
            DriveContent.before_insert(self)
            declared(self)

        before_insert._drive_node_guard = True
        cls.before_insert = before_insert
...
def sweep_unused_media() -> dict:
    ...
    if rows:
        ...
        frappe.cache().set_value(
            _sweep_cursor_key(),
            frappe.as_json({"content_modified": str(last.content_modified), "name": last.name}),
        )
    return {
        "documents": documents,
        "trashed": trashed,
